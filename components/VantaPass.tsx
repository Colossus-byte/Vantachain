...
interface VantaPassProps {
  progress: UserProgress;
}

const VantaPass: React.FC<VantaPassProps> = ({ progress }) => {
  ...
  return (
    <div className="relative group perspective-1000 w-full max-w-md mx-auto">
      <div className="relative h-56 md:h-64 w-full rounded-2xl md:rounded-3xl p-0.5 md:p-1 bg-[#050508] border border-white/10 overflow-hidden transition-all duration-700 hover:rotate-y-12 hover:scale-[1.02] shadow-2xl">
        ...
        {/* Card Content */}
        <div className={`h-full w-full rounded-[1.2rem] md:rounded-[1.4rem] p-6 md:p-8 flex flex-col justify-between relative z-10 transition-all duration-500 ${isPrivate ? 'bg-gradient-to-br from-black to-[#0a0a0f]' : 'bg-gradient-to-br from-white/[0.05] to-transparent'}`}>
          <div className="flex justify-between items-start">
            <div className="flex items-center gap-3 md:gap-4">
              <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full border-2 p-0.5 overflow-hidden transition-all duration-500 ${isPrivate ? 'border-[#8b5cf6] blur-sm' : 'border-[#ccff00]/40'}`}>
                <img src={progress.avatarUrl} alt="ID" className="w-full h-full object-cover rounded-full" />
              </div>
              <div>
                <h3 className={`text-xs md:text-sm font-black uppercase tracking-tighter italic leading-none transition-all duration-500 ${isPrivate ? 'text-[#8b5cf6] opacity-30 select-none' : 'text-white'}`}>
            ...
            <div className={`px-3 md:px-4 py-1 md:py-1.5 rounded-lg border text-[6px] md:text-[8px] font-black uppercase tracking-[0.2em] transition-all duration-500 ${
              isPrivate ? 'border-[#8b5cf6]/30 bg-[#8b5cf6]/10 text-[#8b5cf6]' : `border-${guild.color}-500/30 bg-${guild.color}-500/10 text-${guild.color}-500`
            }`}>
          ...
          <div className="space-y-3 md:space-y-4">
            <div className="flex items-end justify-between">
              ...
              <div className="text-right">
                ...
                <p className="text-lg md:text-xl font-black text-[#ccff00] tracking-tighter italic">{progress.tokenBalance} $PATH</p>
              </div>
            </div>
            ...
            <div className="pt-3 md:pt-4 border-t border-white/5 flex justify-between items-center">
              <div className="flex gap-1 md:gap-1.5">
                {[1, 2, 3, 4, 5].map(i => (
                  <div key={i} className={`w-0.5 md:w-1 h-2 md:h-3 rounded-full ${i <= (progress.completedTopics.length % 5) ? 'bg-[#ccff00]' : 'bg-white/5'}`}></div>
                ))}
              </div>
