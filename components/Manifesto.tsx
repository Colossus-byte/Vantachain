...
const Manifesto: React.FC<ManifestoProps> = ({ onClose }) => {
  const outlineSteps = [
    { title: "Identity Synthesis", desc: "Initialize your VantaPass and sync your neural signature." },
  ...
  return (
    <div className="fixed inset-0 z-[600] bg-[#020205] overflow-y-auto no-scrollbar py-12 md:py-20 px-6">
      <div className="max-w-4xl mx-auto space-y-20 md:space-y-32">
        <header className="text-center space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-12 duration-1000">
          <div className="w-16 h-16 md:w-24 md:h-24 bg-[#ccff00] text-black rounded-2xl md:rounded-3xl mx-auto flex items-center justify-center text-3xl md:text-5xl shadow-[0_0_50px_rgba(204,255,0,0.2)]">
            <i className="fa-solid fa-v"></i>
          </div>
          <h1 className="text-4xl md:text-7xl font-black text-white tracking-tighter italic uppercase">The Vanta Manifesto</h1>
          <p className="text-base md:text-xl text-slate-400 font-medium max-w-2xl mx-auto leading-relaxed">
            The world is centralized by default. VantaChain is your terminal for reclaiming sovereignty.
          </p>
        </header>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-16">
          <div className="space-y-4 md:space-y-6 animate-in fade-in slide-in-from-left-8 duration-1000 delay-300">
            <h2 className="text-[8px] md:text-[10px] font-black text-[#ccff00] uppercase tracking-[0.5em]">The Vision</h2>
          ...
          <div className="space-y-4 md:space-y-6 animate-in fade-in slide-in-from-right-8 duration-1000 delay-300">
            <h2 className="text-[8px] md:text-[10px] font-black text-[#8b5cf6] uppercase tracking-[0.5em]">The Mission</h2>
            <p className="text-xl md:text-3xl font-bold text-white tracking-tight leading-tight">
          ...
          <div className="grid grid-cols-1 gap-4 md:gap-6">
            {outlineSteps.map((step, i) => (
              <div key={i} className="group p-6 md:p-10 rounded-2xl md:rounded-[2.5rem] bg-white/[0.02] border border-white/5 flex flex-col sm:flex-row gap-4 md:gap-8 items-start sm:items-center hover:bg-white/[0.04] transition-all">
                <div className="text-4xl md:text-5xl font-black text-slate-800 italic group-hover:text-[#ccff00] transition-colors">{i + 1}</div>
                <div>
        ...
        <footer className="text-center pb-12 md:pb-20">
          <button 
            onClick={onClose}
            className="w-full sm:w-auto px-12 md:px-20 py-4 md:py-6 bg-[#ccff00] text-black font-black uppercase tracking-[0.2em] text-[10px] md:text-xs rounded-full hover:scale-105 active:scale-95 transition-all shadow-[0_0_40px_rgba(204,255,0,0.3)]"
          >
