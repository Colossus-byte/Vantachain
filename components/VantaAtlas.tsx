...
interface VantaAtlasProps {
  progress: UserProgress;
  onSelectTopic: (id: string) => void;
}

const VantaAtlas: React.FC<VantaAtlasProps> = ({ progress, onSelectTopic }) => {
  return (
    <div className="mb-12 md:mb-24 py-8 md:py-12 px-6 md:px-10 rounded-[2rem] md:rounded-[3.5rem] bg-[#050608] border border-white/5 relative overflow-hidden animate-in fade-in zoom-in-95 duration-1000">
      <div className="absolute top-0 right-0 w-64 md:w-96 h-64 md:h-96 bg-[#ccff00]/5 blur-[80px] md:blur-[120px] pointer-events-none"></div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mb-10 md:mb-16">
        <div>
          ...
          <p className="text-xl md:text-3xl font-black text-white tracking-tighter uppercase italic">The Vanta Atlas</p>
        </div>
          ...
          return (
            <button
              ...
              className={`relative z-10 w-full md:w-64 p-6 md:p-8 rounded-2xl md:rounded-3xl border transition-all duration-500 flex flex-col items-center gap-3 md:gap-4 text-center group ${
                isCompleted ? 'bg-[#ccff00]/10 border-[#ccff00]/40' :
            ...
            >
              <div className={`w-10 h-10 md:w-14 md:h-14 rounded-xl md:rounded-2xl flex items-center justify-center text-base md:text-xl shadow-lg transition-transform duration-500 ${
                isCompleted ? 'bg-[#ccff00] text-black' :
